;;; redtick-autoloads.el --- automatically extracted autoloads (do not edit)   -*- lexical-binding: t -*-
;; Generated by the `loaddefs-generate' function.

;; This file is part of GNU Emacs.

;;; Code:

(add-to-list 'load-path (or (and load-file-name (directory-file-name (file-name-directory load-file-name))) (car load-path)))



;;; Generated autoloads from redtick.el

(defvar redtick-mode nil "\
Non-nil if Redtick mode is enabled.
See the `redtick-mode' command
for a description of this minor mode.")
(custom-autoload 'redtick-mode "redtick" nil)
(autoload 'redtick-mode "redtick" "\
Little pomodoro timer in the mode-line.

This is a global minor mode.  If called interactively, toggle the
`Redtick mode' mode.  If the prefix argument is positive, enable
the mode, and if it is zero or negative, disable the mode.

If called from Lisp, toggle the mode if ARG is `toggle'.  Enable
the mode if ARG is nil, omitted, or is a positive number.
Disable the mode if ARG is a negative number.

To check whether the minor mode is enabled in the current buffer,
evaluate `(default-value \\='redtick-mode)'.

The mode's hook is called both when the mode is enabled and when
it is disabled.

(fn &optional ARG)" t)
(autoload 'redtick "redtick" "\
Enable minor-mode, and start the pomodoro." t)
(autoload 'redtick-with-description "redtick" "\
Ask for DESCRIPTION, enable minor-mode, and start the pomodoro.

(fn DESCRIPTION)" t)
(register-definition-prefixes "redtick" '("redtick-"))

;;; End of scraped data

(provide 'redtick-autoloads)

;; Local Variables:
;; version-control: never
;; no-byte-compile: t
;; no-update-autoloads: t
;; no-native-compile: t
;; coding: utf-8-emacs-unix
;; End:

;;; redtick-autoloads.el ends here
