...
(defvar-local lsp-jt--lens-refresh-fn nil)
               ...
               (t (lsp-jt--create-launch-config
                   ...
                   (lambda ()
                     ...
                     (let (needs-update)
                       ...
                       (when needs-update
                         (condition-case nil (run-hooks 'lsp-jt-status-updated-hook) (error nil))))

                     (-setq (launch-config . launch-configs) launch-configs)
                     ;; start next group if present
                     (when launch-config
                       (lsp-jt--start-group launch-config))
                     (condition-case nil (run-hooks 'lsp-jt-test-run-finished-hook) (error nil)))))))))
         tests))
...
(define-minor-mode lsp-jt-lens-mode
  ...
  (cond
   (lsp-jt-lens-mode
    ...
    (setq-local lsp-jt--lens-refresh-fn
                (let ((buf (current-buffer)))
                  (lambda ()
                    (when (buffer-live-p buf)
                      (with-current-buffer buf
                        (lsp-lens-refresh nil))))))
    (add-hook 'lsp-jt-status-updated-hook lsp-jt--lens-refresh-fn)
    (lsp-lens-refresh t))
   (t
    (setq-local lsp-lens-backends (delete 'lsp-jt-lens-backend lsp-lens-backends))
    (when (bound-and-true-p lsp-jt--lens-refresh-fn)
      (remove-hook 'lsp-jt-status-updated-hook lsp-jt--lens-refresh-fn)))))
                ...
                (unless cleanup-ran
                  ...
                  (run-with-idle-timer
                   ...
                   (lambda ()
                     (condition-case err
                         (progn
                           (condition-case nil (funcall analyzer "\n") (error nil))
                           (when tcp-server-process
                             (condition-case nil (delete-process tcp-server-process) (error nil)))
                           (funcall finished-function))
                       (error (message "JT: idle-timer error: %S" err))))))))))))
       ...
       (lambda (item)
         (cond
          ...
          ((s-starts-with? lsp-jt--junit-test-end item)
           (-let* ((test-id (lsp-jt--extract-name item project))
                   (test-data (gethash test-id result))
                   (old-status (plist-get test-data :status))
                   (old-start (plist-get test-data :start)))
             (when test-data
               (puthash test-id
                        (list :start old-start
                              :duration (- (time-to-seconds (current-time)) (or old-start 0))
                              :status (if (or (eq old-status :running) (null old-status))
                                          :pass
                                        old-status)
                              :traces (when traces (s-join "\n" (nreverse traces))))
                        result)
