...
** Try to try out emacs packages

   Install try to allow me to experiment with packages.
...
** Set initial framesize
   ...
   #+BEGIN_SRC emacs-lisp
     (setq initial-frame-alist '((top . 0) (left . 0) (width . 200)
                                      (height . 100)))
     (add-to-list 'default-frame-alist '(height . 100)) ;; Vertical frame size
     (add-to-list 'default-frame-alist '(width . 200)) ;; Horizontal frame size
   #+END_SRC
...
** loccur mode
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (global-set-key (kbd "M-o") 'loccur-current)
...
** window-number-super mode
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package window-number
       :ensure t)
     ;; Load window-number and unset C-x C-j
     (with-eval-after-load "window-number"
       (define-key window-number-mode-map (kbd "C-x C-j") nil))
...
** Set auth sources to use gpg
   ...
   Set auth sources to use the .authinfo.gpg file. Used by tools like Forge.
...
** command key as meta
   ...
   Function to remap meta and super keys for Mac keyboard on linux and not mac.

   #+begin_src emacs-lisp
     (defun linux-mac-keyboard-remap()
       "Remap meta and super to suit an external Mac keyboard on a linux pc and map meta window keys to super-n"
       (interactive)
       (setq x-meta-keysym 'super)
       (setq x-super-keysym 'meta))
   #+end_src

   Function to remap meta and super keys for normal keyboard on linux and not mac.

   #+begin_src emacs-lisp
     (defun normal-keyboard-remap()
       "Map meta and super to suit a normal keyboard on a linux pc"
       (interactive)
...
*** Modus themes
    ...
    #+begin_src emacs-lisp :tangle yes
      (use-package modus-themes
        :ensure t
        :init (load-theme 'modus-vivendi-deuteranopia :no-confirm))
    #+end_src
...
** Apheleia
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (with-eval-after-load 'apheleia
       (setf (alist-get 'yaml-mode apheleia-mode-alist nil 'remove) nil)
       (setf (alist-get 'yaml-ts-mode apheleia-mode-alist nil 'remove) nil))
   #+END_SRC
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package company
       ...
       (setq-default company-tooltip-limit 30                         ; bigger popup window
                     ...
                     ;; company-preview-if-just-one-frontend is needed: the tooltip frontend
                     ;; hides its dropdown when exactly one candidate remains (e.g. after typing
                     ;; enough of a method name to disambiguate it), so without the preview
                     ;; frontend that single-candidate case shows nothing at all. Its inline
                     ;; ghost text can visually collide with copilot's overlay; that's handled
                     ;; by suppressing copilot's overlay whenever company has any candidates
                     ;; (see copilot-disable-predicates in the Copilot section) rather than by
                     ;; dropping this frontend.
...
*** magit
    ...
    #+BEGIN_SRC emacs-lisp
      (use-package magit
        ...
        (unless (boundp 'magit-auto-revert-mode)
          (custom-initialize-set 'magit-auto-revert-mode
...
** Pandoc mode
   ...
   Pandoc mode for viewing html, pdf, markdown, etc.
...
** Markdown mode
   ...
   #+begin_src emacs-lisp
     ...
     (defun chris/markdown-export-pdf ()
       "Convert the current markdown buffer to PDF using pandoc."
       (interactive)
       ...
       (let* ((file (buffer-file-name))
         ...
         (compile (format "pandoc %s -o %s"
                          (shell-quote-argument file)
                          (shell-quote-argument pdf-file)))))

     (with-eval-after-load 'markdown-mode
       (define-key markdown-mode-map (kbd "C-c C-e p") #'chris/markdown-export-pdf))
   #+end_src
...
*** Openapi mode
    ...
    #+begin_src emacs-lisp :tangle yes
      (use-package openapi-mode
        :vc (openapi :url "https://github.com/xshyamx/openapi-mode" :rev :newest)
        :hook (yaml-mode . openapi-mode))
    #+end_src
...
** Mermaid in markdown
   ...
   #+begin_src emacs-lisp :tangle yes
     (use-package markdown-mermaid
       :vc (markdown-mermaid :url "https://github.com/pasunboneleve/markdown-mermaid" :rev :newest)
...
*** LSP config
    ...
    #+begin_src emacs-lisp
      (use-package lsp-mode
        :ensure t
        :init ; set prefix for lsp-command-keymap - use meta-super-l ("s-l")
...
*** LSP for Java
    ...
    #+begin_src emacs-lisp
      (defun my/vscode-ext-path (glob)
        "Return the newest directory under ~/.vscode/extensions matching GLOB."
        (car (last (sort (file-expand-wildcards
                          (expand-file-name glob "~/.vscode/extensions"))
                         #'string<))))
      ...
      (use-package lsp-java
        ...
        (require 'dap-java)
        (require 'lsp-jt)
        (setq lsp-java-bundles nil)
        (setq lsp-jt-root "~/.emacs.d/.cache/lsp/eclipse.jdt.ls/java-test/extension/server/")
        (setq lsp-java-vmargs
              (list "-XX:+UseParallelGC"
                    ...
                    "-Xms100m"
                    (concat "-javaagent:"
                            (expand-file-name "~/.m2/repository/org/projectlombok/lombok/1.18.46/lombok-1.18.46.jar"))))
        (setq lsp-java-test-config '(:vmArgs ["-XX:+EnableDynamicAgentLoading"]))
        (add-hook 'lsp-mode-hook #'lsp-lens-mode)
        (add-hook 'lsp-configure-hook #'lsp-jt-lens-mode)
        (add-hook 'lsp-jt-test-run-finished-hook #'lsp-jt--schedule-refresh-lens)
        (add-hook 'lsp-jt-test-run-finished-hook #'lsp-jt-report-open)
        ...
        (add-hook 'lsp-configure-hook
                  (lambda ()
                    ;; Force Emacs to use normal CC-mode indentation logic for regions
                    ;; instead of handing control over to the LSP server
                    (setq-local indent-region-function #'c-indent-region)
                    (setq-local ))))

      (advice-add 'lsp--error :around
                  (lambda (orig fmt &rest args)
                    (unless (string-match "vscode\\.java\\.test"
    ...
    Keep codeium out of Java buffers -- copilot already provides inline AI
    suggestions there, and codeium's capf would just double up with it
    without adding anything (unlike kotlin/typescript/clojure, java has no
    codeium+lsp capf combo to begin with).

    #+begin_src emacs-lisp
      (add-hook 'java-mode-hook
                (lambda ()
                  (setq-local completion-at-point-functions
                              (list #'lsp-completion-at-point #'cape-dabbrev #'cape-file))))
    #+end_src
    ...
    #+begin_src emacs-lisp
      (defun bind-java-dap-keys ()
        (define-key java-mode-map (kbd "s-l t t") 'dap-java-run-test-method)
        (define-key java-mode-map (kbd "s-l t d") 'dap-java-debug-test-method)
        (define-key java-mode-map (kbd "s-l t c") 'dap-java-run-test-class))
      (with-eval-after-load 'cc-mode (bind-java-dap-keys))
    #+end_src
...
** Codeium
   ...
   #+begin_src emacs-lisp
     ...
     (use-package codeium
       ...
       ;; (setq codeium/metadata/api_key anthropic-key)
...
*** Add in copilot mode.
    ...
    #+begin_src emacs-lisp
      (use-package copilot
        ...
        :config
        ;; Don't pop up copilot's ghost text while company has any candidates
        ;; live -- whether shown as a dropdown or (for a single remaining
        ;; candidate) as company's own inline preview -- to avoid the two
        ;; overlays overlapping.
        (add-to-list 'copilot-disable-predicates (lambda () (bound-and-true-p company-candidates)))
        ;; Disable copilot in scratch and temp buffers
        (add-to-list 'copilot-disable-predicates
               (lambda () (string-prefix-p "*" (buffer-name))))
        ;; Clojure has no single indent-width variable the way python/lisp do
        ;; (clojure-mode indents semantically), so copilot--infer-indentation-offset
        ;; warns it found nothing. 2 spaces matches standard Clojure style.
        (dolist (mode '(clojure-mode clojurec-mode clojurescript-mode))
          (add-to-list 'copilot-indentation-alist (list mode 2))))
    #+end_src
...
** Gptel
   ...
   Add in GPTel mode to allow integration of any AI.
...
** Claude Code
   ...
   Use Claude code in Emacs.

   #+begin_src emacs-lisp :tangle yes
     (use-package claude-code-ide
       :vc (claude-code-ide :url "https://github.com/manzaltu/claude-code-ide.el" :rev :newest)
    ...
    #+begin_src emacs-lisp
      (use-package clojure-mode
        ...
        (clojure-mode .
                      (lambda ()
                        (setq-local completion-at-point-functions
                                    (list (cape-capf-super #'lsp-completion-at-point #'codeium-completion-at-point))))))
    #+end_src
...
*** Erlang mode
    ...
    #+begin_src emacs-lisp
      (setq load-path (cons "/home/chris/.kerl/builds/27.3/otp_src_27.3/lib/tools/emacs" load-path))
      (defun set-erlang-completion
          ()
        (interactive)
        (setq lsp-completion-enable nil)
        (setq-local
      ...
      (use-package erlang :load-path
        ("~/.kerl/builds/27.3/otp_src_27.3/lib/tools/emacs")
        :mode
        (("\\.erl?$" . erlang-mode)
        ...
        (setq erlang-root-dir "~/.kerl/builds/27.3/otp_src_27.3/")
        (setq exec-path (cons "~/.kerl/builds/27.3/otp_src_27.3/bin" exec-path))
        (setq erlang-man-root-dir "~/.kerl/builds/27.3/otp_src_27.3/man")
        (add-hook 'erlang-mode-hook 'my-paredit-nonlisp)
        (add-hook 'erlang-mode-hook 'set-erlang-completion)
        )
    #+end_src
...
** Python
   ...
   Python mode is installed by default but I want to add hooks for code completion


*** LSP

    Install the ms lsp server.
    ...
    #+begin_src emacs-lisp :tangle yes
      (use-package lsp-python-ms
        ...
        (setq lsp-python-ms-executable
              "~/python-projects/python-language-server/output/bin/Release/osx-x64/publish/Microsoft.Python.LanguageServer")
        )
    #+end_src
...
*** Completion
    ...
    #+begin_src emacs-lisp
      ...
      ;;           (lambda ()
      ;;             (setq-local completion-at-point-functions
      ;;                         (list (cape-capf-super #'codeium-completion-at-point)))))
    #+end_src

*** Anaconda mode

    Add anaconda mode for code completion and code navigation.
...
*** Nose for testing
    ...
    Add nose mode for running nose tests.

    #+begin_src emacs-lisp :tangle yes
      (use-package nose
        :vc (nose :url "https://github.com/syl20bnr/nose.el")
        :config
        ;; (add-hook 'python-mode-hook
        ;;           (lambda ()
...
*** Pytest for testing
    ...
    Add pytest mode for testing with pytest

    #+begin_src emacs-lisp :tangle yes
      (use-package pytest
        :vc (pytest :url "https://github.com/ionrock/pytest-el")
        :config
        (add-hook 'python-mode-hook
                  (lambda ()
...
*** Kotlin Mode
    ...
    Set up Kotlin mode for editing Kotlin files.
...
*** Gradle Mode
    ...
    Add support for Gradle build tool commonly used with Kotlin projects.
...
*** Flycheck Kotlin
    ...
    Setup Flycheck for Kotlin syntax checking.
...
*** LSP for Kotlin
    ...
    Configure LSP support for Kotlin using the Kotlin Language Server.
    Make sure to install the Kotlin Language Server separately.

    #+BEGIN_SRC emacs-lisp
      ;; Customize some kotlin-specific LSP settings if needed
      (with-eval-after-load 'lsp-mode
        (setq lsp-kotlin-compiler-jvm-target "11")
...
*** Kotlin REPL
    ...
    Setup REPL for interactive Kotlin development.

    #+BEGIN_SRC emacs-lisp
      (defun kotlin-repl ()
        "Run Kotlin REPL in a comint buffer."
        (interactive)
        (let ((buffer (comint-check-proc "Kotlin REPL")))
          (unless buffer
            (setq buffer (make-comint "Kotlin REPL" "kotlinc" nil))
      ...
      (defun kotlin-send-region (start end)
        "Send region to Kotlin REPL."
        (interactive "r")
        (let ((code (buffer-substring-no-properties start end)))
          (let ((buffer (get-buffer-create "*Kotlin REPL*")))
            (unless (comint-check-proc buffer)
              (kotlin-repl))
            (comint-send-string buffer (concat code "\n")))))

      (defun kotlin-send-buffer ()
        "Send current buffer to Kotlin REPL."
        (interactive)
        (kotlin-send-region (point-min) (point-max)))

      (defun kotlin-send-line ()
        "Send current line to Kotlin REPL."
        (interactive)
        (kotlin-send-region (line-beginning-position) (line-end-position)))

      (add-hook 'kotlin-mode-hook
                (lambda ()
                  (local-set-key (kbd "C-c C-z") 'kotlin-repl)
                  (local-set-key (kbd "C-c C-r") 'kotlin-send-region)
                  (local-set-key (kbd "C-c C-c") 'kotlin-send-buffer)
                  (local-set-key (kbd "C-c C-l") 'kotlin-send-line)))
    #+END_SRC

*** DAP for Kotlin

    Configure Debug Adapter Protocol for Kotlin.

    #+BEGIN_SRC emacs-lisp
      (with-eval-after-load 'dap-mode
        (require 'dap-java)  ;; Kotlin can use Java debugger

        ;; Custom template for Kotlin debugging
        (dap-register-debug-template "Kotlin App"
                                     (list :type "java"
                                           :request "launch"
                                           :args ""
                                           :cwd nil
...
*** Kotlin Snippets
    ...
    Add Kotlin code snippets for quicker development.

    #+BEGIN_SRC emacs-lisp
      (with-eval-after-load 'yasnippet
        (add-to-list 'yas-snippet-dirs (concat user-emacs-directory "snippets/kotlin"))
        (yas-reload-all))
    #+END_SRC

*** Projectile Integration for Kotlin Projects

    Configure Projectile to recognize Kotlin projects.

    #+BEGIN_SRC emacs-lisp
      (projectile-register-project-type 'kotlin-gradle '("build.gradle.kts")
...
*** Haskell mode
    ...
    #+begin_src emacs-lisp :tangle yes
      (use-package haskell-mode
        :ensure t
        :hook ((haskell-mode . my-paredit-nonlisp)
               (haskell-mode . lsp)
               (haskell-mode .
                  (lambda ()
                    (setq-local completion-at-point-functions
...
** EditorConfig
   ...
   #+begin_src emacs-lisp
     (use-package editorconfig
          ...
          :config
          (editorconfig-mode 1))
     ;; Bind EditorConfig variables explicitly to Java CC-mode hooks
     (add-hook 'editorconfig-after-apply-functions
               (lambda (props)
                 (when (derived-mode-p 'java-mode)
                   (let ((indent-size (gethash 'indent_size props)))
                     (when indent-size
                       (setq c-basic-offset (string-to-number indent-size)))))))
   #+end_src
...
** envrc so that direnv runs

   I want to user direnv to set env vars per project.
