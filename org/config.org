...
*** clojure
    ...
    Compojure's route macros (GET/POST/PUT/DELETE/ANY) aren't known to
    clojure-mode, so with the default clojure-indent-style ('always-align)
    it treats them as plain function calls and aligns their body under the
    first argument instead of the usual 2-space macro-body indent. This
    only matters for buffer-local reindenting (e.g. paredit-reindent-defun,
    which uses clojure-mode's own indent-line-function); clojure-lsp's
    formatter already knows about them and gets it right regardless.
