   ...
   #+BEGIN_SRC emacs-lisp
     (use-package company
       ...
       (setq-default company-tooltip-limit 30                         ; bigger popup window
                     ...
                     ;; NOTE: company-preview-if-just-one-frontend draws inline "ghost text",
                     ;; which visually collides with copilot's overlay suggestions. Dropped
                     ;; so company only ever shows the tooltip, leaving ghost-text to copilot.
...
*** Add in copilot mode.
    ...
    #+begin_src emacs-lisp
      (use-package copilot
        ...
        :config
        ;; Don't pop up copilot's ghost text while company's dropdown is
        ;; already showing candidates -- avoids the two UIs overlapping.
        (add-to-list 'copilot-disable-predicates (lambda () (company-tooltip-visible-p))))
    #+end_src
