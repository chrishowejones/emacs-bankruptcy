...
** Cape
   ...
   Use Cape (Completion at point extensions). At the moment I just need the cache buster fn to reset completions when you delete chars.
...
** Corfu
   ...
   Try Corfu as a completion mode in replacement for company.
   ...
   #+begin_src emacs-lisp :tangle yes
     (use-package corfu
       ...
       (corfu-auto-prefix 2)          ;; Type 2 chars to start autocomplete
       (corfu-auto-delay 0.2)         ;; 0.2 seconds wait for autocomplete
...
** Comment-dwim for prog modes and org-mode

   Use comment do what I mean bound to C-; in all prog modes.
   ...
   and org-mode (for org-babel src blocks).
